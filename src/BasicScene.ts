...
export default class BasicScene extends THREE.Scene {
   ...
   */
  initialize(debug = true, addGridHelper = true): void {
    // setup camera
    ...
    for (let i = 0; i < this.lightCount; i += 1) {
      ...
      const lightX = this.lightDistance * Math.sin(((Math.PI * 2) / this.lightCount) * i);
      const lightZ = this.lightDistance * Math.cos(((Math.PI * 2) / this.lightCount) * i);
      // Create a light
   ...
   */
  static addWindowResizing(camera: THREE.PerspectiveCamera, renderer: THREE.Renderer): void {
